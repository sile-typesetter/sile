...
SILE.registerUnit = function (unit, spec)
  ...
  if type(def) == "string" then
    ...
    if not SILE.units[baseunit] then SU.error("Unit "..unit.." defined in terms of unknown unit "..baseunit) end
    if SILE.units[baseunit].relative then
      SILE.units[unit] = { relative = true, convertor = function (value) return factor * SILE.toPoints(value, baseunit) end}
    else
...
SILE.toPoints = function(factor, unit)
  if (not unit) then
    ...
    else return tonumber(factor) end
  end
