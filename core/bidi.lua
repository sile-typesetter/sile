...
function bidi.splitNodelistIntoBidiRuns(nl, direction)
   if #nl == 0 then
      return nl
   end
   local owners, text = nodeListToText(nl)
   local base_level = direction == "RTL" and 1 or 0
   local runs = { icu.bidi_runs(table.concat(text), direction) }
...
function bidi.reorder(nl, direction)
   -- local newNl = {}
   -- local matrix = {}
   local levels = {}
   local base_level = direction == "RTL" and 1 or 0
