    SILE.registerCommand("font", function(options, content)
      ...
      if (options.width)  then SILE.settings.set("font.width", 0+options.width) end
    ...
    end, "Set current font family, size, weight, width, style, variant, script, direction and language")
...
SILE.settings.declare({name = "font.width", type = "number or integer", default = 100})
...
local _key = function(options)
  return table.concat({options.font;("%g"):format(options.size);("%d"):format(options.weight);("%g"):format(options.width);options.style;options.variant;options.features;options.direction;options.filename},";")
end
...
SILE.font = {loadDefaults = function(options)
  ...
  if not options.width then options.width = SILE.settings.get("font.width") end
