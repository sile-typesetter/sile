...
(by drawing a solid color block the full size of the page on page initialization).
The package provides a \code{\\background} command which requires at least one
parameter, \code{color=\em{<color \nobreak{}specification}}, and sets the backgound
of the current and all following pages to that color. If setting only the current
page background different from the default is desired, an extra parameter
...
So, for example, \code{\\background[color=#e9d8ba]} will set a sepia tone background.
\end{document}]] }
