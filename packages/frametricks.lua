...
SILE.registerCommand("float", function (options, content)
  ...
  if SILE.length.make(heightOfPageSoFar + hbox.height - SILE.typesetter:getTargetLength()).length > 0 then
    SILE.call("eject")
    SILE.typesetter:leaveHmode()
  end
  breakFrameVertical()
  local boundary = hbox.width + SILE.length.parse(options.rightboundary):absolute()
  ...
  SILE.settings.set("document.baselineskip", SILE.settings.get("document.baselineskip").height:negate() + SILE.length("1ex"))
  -- undoSkip.stretch = hbox.height
  -- SILE.typesetter:pushHbox({ value = {} })
  -- SILE.typesetter:pushVglue({ height = undoSkip })
  breakFrameVertical(hbox.height + SILE.length.parse(options.bottomboundary):absolute())
