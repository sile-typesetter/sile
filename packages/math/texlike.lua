...
local mathGrammar = function(_ENV)
  ...
  local element_no_infix = V("def") + V("command") + group + V("argument") + V("atom")
  local element = V("supsub") + V("subsup") + V("sup") + V("sub") + element_no_infix
  local sep = S(",;") * _
  local quotedString = (P('"') * C((1 - P('"')) ^ 1) * P('"'))
  local value = (quotedString + (1 - S(",;]")) ^ 1)
  local pair = Cg(sileID * _ * "=" * _ * C(value))
    * sep ^ -1
    / function(...)
      local t = { ... }
      return t[1], t[#t]
  ...
  local parameters = (P("[") * list * P("]")) ^ -1 / function(a)
  ...
  local dim2_arg = Cg(P("{") * dim2_arg_inner * (P("\\\\") * dim2_arg_inner) ^ 1 * (P("}") + E("`}` expected")))
    / function(...)
      local t = { ... }
      -- Remove the last mathlist if empty. This way,
      -- `inner1 \\ inner2 \\` is the same as `inner1 \\ inner2`.
      if not t[#t][1] or not t[#t][1][1] then
        table.remove(t)
      end
  ...
  supsub = element_no_infix * _ * P("^") * _ * element_no_infix * _ * P("_") * _ * element_no_infix
  subsup = element_no_infix * _ * P("_") * _ * element_no_infix * _ * P("^") * _ * element_no_infix
  sup = element_no_infix * _ * P("^") * _ * element_no_infix
  sub = element_no_infix * _ * P("_") * _ * element_no_infix
  atom = natural + C(utf8code - S("\\{}%^_&")) + (P("\\{") + P("\\}")) / function(s)
    return string.sub(s, -1)
  end
  command = (P("\\") * Cg(ctrl_sequence_name, "command") * Cg(parameters, "options") * (dim2_arg + group ^ 0))
  def = P("\\def")
    * _
    * P("{")
    * Cg(ctrl_sequence_name, "command-name")
    * P("}")
    * _
