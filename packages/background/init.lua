...
local outputBackground = function (background)
  ...
  if type(background.bg) == "string" then
    SILE.outputter:drawImage(background.bg, bleedarea:left(), bleedarea:top(), bleedarea:width(), bleedarea:height())
...
function package:registerCommands ()
  ...
  self:registerCommand("background", function (options, _)
    if SU.boolean(options.disable, false) then
      -- This options is certainly better than enforcing a white color.
...
As its name implies, the \autodoc:package{background} package allows you to set
the color of the page canvas background or to use a background image extending
to the full page width and height.

The package provides a \autodoc:command{\background} command which
requires one of the following parameters:
\begin{itemize}
\item{\autodoc:parameter{color=<color specification>} sets the background of the
current and all following pages to that color. The color specification has the same
syntax as specified in the \autodoc:package{color} package.}
\item{\autodoc:parameter{src=<file>} sets the backgound of the
current and all following pages to the specified image. The latter will be scaled
to the target dimension.}
\end{itemize}

The background extends to the page trim area (“page bleed”) if the latter is defined.
This is to ensure that it indeed “bleeds” off the sides of the page, so as to
avoid thin white lignes on an otherwise full color page when the paper sheet is cut
to dimension but some pages are trimmed slightly more than others.

If setting only the current page background different from the default is desired,
an extra parameter \autodoc:parameter{allpages=false} can be passed.
...
The \autodoc:parameter{disable=true} parameter allows disabling the background on the
following pages. It may be useful when \autodoc:parameter{allpages} is active from a
previous invocation.
\end{document}
]]
