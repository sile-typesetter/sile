...
local colon = P(":")
local slash = P("/")
local yapprox = P("?") / function () return "true" end
local mapprox = P("~") / function () return "true" end
local dapprox = P("%") / function () return "true" end
-- time
local D2 = digit * digit / tonumber
local offset = P("Z") + C(S("+-")) * C(D2) * colon * C(D2) / function (s, h, m)
   local sign = s == "+" and 1 or -1
      return { hour = h * sign, minute = m * sign }
   end
local timespec = P("T") * Cg(D2, "hour") * colon * Cg(D2, "minute") * colon * Cg(D2, "second") * Cg(offset^-1, "offset")
...
local season = P("2") * R("14") / function (s) return tonumber(s) - 20 end
...
local date = Ct(datespec) / function (t)
   return {
      year = t.year,
      season = t.season,
      month = t.month,
...
local startdate = Ct(date * slash * date^-1) / function (t)
...
local enddateonly = slash * date / function (t)
  return {
