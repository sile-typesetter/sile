...
function pango.getFace (options)
  ...
  if palcache[p] then return palcache[p]
    ...
    if options.language then pal:insert(pangolgi.Attribute.language_new(pangolgi.Language.from_string(options.language))) end
    if options.font then pal:insert(pangolgi.Attribute.family_new(options.font)) end
    if options.weight then pal:insert(pangolgi.Attribute.weight_new(tonumber(options.weight))) end
    if options.size then pal:insert(pangolgi.Attribute.size_new(options.size * 1024 * 0.75)) end -- I don't know why 0.75
    if options.style then pal:insert(pangolgi.Attribute.style_new(
      options.style:lower() == "italic" and pangolgi.Style.ITALIC or pangolgi.Style.NORMAL)) end
    if options.variant then pal:insert(pangolgi.Attribute.variant_new(
      options.variant:lower() == "smallcaps" and pangolgi.Variant.SMALL_CAPS or pangolgi.Variant.NORMAL)) end
  end
