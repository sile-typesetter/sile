...
describe("#SIL #input parser", function ()
  ...
  describe("should handle", function ()
    ...
    it("commands with multiple args", function()
      local t = inputter:docToTree([[\foo[baz=qiz,qiz=baz]{bar}]])
      assert.is.equal("foo", t.command)
      assert.is.equal("qiz", t.options.baz)
    ...
    it("commands with quoted arg", function()
      local t = inputter:docToTree([[\foo[baz="qiz qiz"]{bar}]])
      assert.is.equal("foo", t.command)
    ...
    it("commands with multiple quoted args", function()
      local t = inputter:docToTree([[\foo[baz="qiz, qiz",qiz="baz, baz"]{bar}]])
      assert.is.equal("foo", t.command)
      assert.is.equal("qiz, qiz", t.options.baz)
  ...
  describe("should reject", function ()
    ...
    it("commands with bad characters", function()
      assert.has.error(function() inputter:docToTree([[\"]]) end)
      assert.has.error(function() inputter:docToTree([[\']]) end)
      assert.has.error(function() inputter:docToTree([[\"o]]) end)
    end)

    it("commands with unclosed content", function()
      assert.has.error(function() inputter:docToTree([[\foo{bar]]) end)
    end)

    it("unclosed environments", function()
      assert.has.error(function() inputter:docToTree([[\begin{foo}bar]]) end)
    end)

    it("mismatched environments", function()
      assert.has.error(function() inputter:docToTree([[\begin{foo}\begin{bar}baz\end{foo}\end{bar}]]) end)
    end)
