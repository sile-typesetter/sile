local t
pcall(function() t = require("justenoughthai") end)
if t then
  SILE.nodeMakers.th = SILE.nodeMakers.base {
    iterator = function (self, items)
      ...
      local chunks = {t.breakpoints(fulltext)}
      self:init()
      ...
      return coroutine.wrap(function()
        for i = 1,#items do item = items[i]
          ...
          if chunks[1] and (items[i].index >= chunks[1]) then
            -- There's a break here
            local bp = chunks[1]
            while chunks[1] and items[i].index >= chunks[1] do
              table.remove(chunks,1)
            end
            self:makeToken()
            self:makePenalty(0)
