...
local icuFormat = function (number, lang, options)
   ...
   local ok, result = pcall(icu.format_number, number, iculocale, icustyle)
   if ok and options.system and icustyle == 0 and options.system ~= "latn" and result == tostring(number) then
      -- There are valid cases where "@numbers=xxxx" with default(0) and decimal(1) styles both work.
      -- Typically, with number=1234
      ...
      ok, result = pcall(icu.format_number, number, "@numbers=" .. options.system, 1)
   end
   if not ok then
      SU.warn("Number formatting failed: " .. tostring(result))
   end
   return tostring(ok and result or number)
end
...
function language:formatNumber (number, options, case)
   if math.abs(number) > 9223372036854775807 then
   ...
   local result
   if style and type(self[self._numberingMethod(style)]) == "function" then
      result = self[self._numberingMethod(style)](self, number, options)
   elseif system and type(self[self._numberingMethod(system)]) == "function" then
      result = self[self._numberingMethod(system)](self, number, options)
   else
      result = icuFormat(number, lang, options)
   end
...
function language:numberToAlpha (number)
   local out = ""
   local a = string.byte("a")
   repeat
      number = number - 1
      out = string.char(number % 26 + a) .. out
